      ...
      if (!category) {
        throw new AstroError(
          ...
          `A media item references a non-existent category: "${c.id}".\n` +
            ...
            `src/content/categories/${c.id}.yaml`,
        )
      }
...
export async function getCategory(id: string) {
  ...
  if (!category) {
    throw new AstroError(
      `Missing category "${id}"`,
      `To fix the issue, add a category at "src/content/categories/${id}.yaml".`,
    )
  }
  return {
    id,
...
function parseCategory(item: unknown) {
  return verifySchemaAsync(
    ...
    (id) => `Fix these issues inside "src/content/categories/${id}.yaml":`,
  )
}
